...
OUT_DIR = ROOT / "analysis_outputs" / "combined_hotelling_t2"
PHYS_PATH = (
    ROOT
    / "analysis_outputs"
...
def fit_direction_and_scale(train: pd.DataFrame, features: list[str]) -> tuple[pd.Series, pd.Series, pd.Series]:
...
def orient_and_scale(
    ...
    oriented = ((x - normal_median) / scale) * direction
    oriented.columns = features
    return oriented


def fit_class_covariance(x_train: pd.DataFrame, y_train: np.ndarray) -> dict[str, object]:
    imputer = SimpleImputer(strategy="median")
    x = imputer.fit_transform(x_train)
    normal = x[y_train == 0]
    impaired = x[y_train == 1]
    normal_cov = LedoitWolf().fit(normal)
    impaired_cov = LedoitWolf().fit(impaired)
    pooled_cov = LedoitWolf().fit(x)
    return {
        "imputer": imputer,
        "normal_center": normal_cov.location_,
        "impaired_center": impaired_cov.location_,
        "normal_precision": normal_cov.precision_,
        "impaired_precision": impaired_cov.precision_,
        "pooled_precision": pooled_cov.precision_,
    }


def t2_distance(x: np.ndarray, center: np.ndarray, precision: np.ndarray) -> np.ndarray:
    delta = x - center
    return np.maximum(np.einsum("ij,jk,ik->i", delta, precision, delta), 0.0)


def transform_t2(x_df: pd.DataFrame, cov: dict[str, object]) -> pd.DataFrame:
    x = cov["imputer"].transform(x_df)
    normal_center = cov["normal_center"]
    impaired_center = cov["impaired_center"]
    t2_normal = t2_distance(x, normal_center, cov["normal_precision"])
    t2_impaired = t2_distance(x, impaired_center, cov["impaired_precision"])
    t2_normal_pooled = t2_distance(x, normal_center, cov["pooled_precision"])
    t2_impaired_pooled = t2_distance(x, impaired_center, cov["pooled_precision"])
    out = pd.DataFrame(index=x_df.index)
    out["t2_normal"] = t2_normal
    out["t2_impaired"] = t2_impaired
    out["t2_margin_normal_minus_impaired"] = t2_normal - t2_impaired
    out["t2_normal_pooled"] = t2_normal_pooled
    out["t2_impaired_pooled"] = t2_impaired_pooled
    out["t2_margin_pooled_normal_minus_impaired"] = t2_normal_pooled - t2_impaired_pooled
    out["t2_normal_log"] = np.log1p(t2_normal)
    out["t2_impaired_log"] = np.log1p(t2_impaired)
    out["t2_margin_log_normal_minus_impaired"] = out["t2_normal_log"] - out["t2_impaired_log"]
    return out
...
def run_feature_set(name: str, features: list[str], table: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    splitter = StratifiedShuffleSplit(n_splits=100, test_size=0.2, random_state=1010000)
    metrics_rows = []
    pred_rows = []
    model_sets = {
        "t2_class_cov_all": [
        ...
        "t2_class_margin_only": ["t2_margin_log_normal_minus_impaired"],
        "t2_pooled_all": [
            "t2_normal_pooled",
            "t2_impaired_pooled",
            "t2_margin_pooled_normal_minus_impaired",
        ],
        "t2_pooled_margin_only": ["t2_margin_pooled_normal_minus_impaired"],
    }
    for repeat, (train_idx, test_idx) in enumerate(splitter.split(table, y)):
        ...
        normal_median, scale, direction = fit_direction_and_scale(train, features)
        x_train = orient_and_scale(train, features, normal_median, scale, direction)
        x_test = orient_and_scale(test, features, normal_median, scale, direction)
        cov = fit_class_covariance(x_train, y[train_idx])
        t2_train = transform_t2(x_train, cov)
        t2_test = transform_t2(x_test, cov)
        for model_name, cols in model_sets.items():
            clf = logistic(1020000 + repeat)
            clf.fit(t2_train[cols], y[train_idx])
            train_prob = clf.predict_proba(t2_train[cols])[:, 1]
            threshold = threshold_youden(y[train_idx], train_prob)
            test_prob = clf.predict_proba(t2_test[cols])[:, 1]
...
def predict_samples(feature_set: str, features: list[str], table: pd.DataFrame) -> pd.DataFrame:
    ...
    y = table["target"].astype(int).to_numpy()
    normal_median, scale, direction = fit_direction_and_scale(table, features)
    x_train = orient_and_scale(table, features, normal_median, scale, direction)
    x_sample = orient_and_scale(samples, features, normal_median, scale, direction)
    cov = fit_class_covariance(x_train, y)
    t2_train = transform_t2(x_train, cov)
    t2_sample = transform_t2(x_sample, cov)
    model_sets = {
        "t2_class_cov_all": [
            "t2_normal_log",
            "t2_impaired_log",
    ...
    rows = []
    for model_name, cols in model_sets.items():
        clf = logistic(1030000 + len(feature_set) + len(model_name))
        clf.fit(t2_train[cols], y)
        train_prob = clf.predict_proba(t2_train[cols])[:, 1]
        threshold = threshold_youden(y, train_prob)
        sample_prob = clf.predict_proba(t2_sample[cols])[:, 1]
        for idx, sample in samples.iterrows():
            ...
            for col in t2_sample.columns:
                row[col] = t2_sample.loc[idx, col]
            rows.append(row)
...
def main() -> None:
    ...
    metrics_df.to_csv(OUT_DIR / "hotelling_t2_metrics_by_repeat.csv", index=False, encoding="utf-8-sig")
    preds_df.to_csv(OUT_DIR / "hotelling_t2_predictions_by_repeat.csv", index=False, encoding="utf-8-sig")
    samples_df.to_csv(OUT_DIR / "hotelling_t2_sample_predictions.csv", index=False, encoding="utf-8-sig")
    ...
    summary.to_csv(OUT_DIR / "hotelling_t2_metrics_summary.csv", index=False, encoding="utf-8-sig")
    ...
    dataset_summary.to_csv(OUT_DIR / "hotelling_t2_dataset_summary.csv", index=False, encoding="utf-8-sig")

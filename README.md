...
**일상보행 + 임상 OR 라벨 기반 acc-only 3피처 subwindow 집계 모델**
...
**평가 방식**: StratifiedGroupKFold 5-fold, 100회 반복 (group = subject_id, threshold = 0.345)
...
- 100회 반복 AUC 95% CI: 약 0.821 ~ 0.901
- seed=42 OOF 기준 threshold=0.345, sensitivity=0.943, specificity=0.722

**단일 fold 예시 (5-fold 1회 고정 기준)**

| Fold | Train AUC | Train Sen | Train Spec | Test AUC | Test Sen | Test Spec |
|------|-----------|-----------|------------|----------|----------|-----------|
| 1 | 0.885 | 0.793 | 0.704 | 0.889 | 1.000 | 0.778 |
| 2 | 0.896 | 0.808 | 0.774 | 0.756 | 0.889 | 0.600 |
| 3 | 0.867 | 0.857 | 0.724 | 0.959 | 1.000 | 0.714 |
| 4 | 0.914 | 0.897 | 0.750 | 0.813 | 0.500 | 0.750 |
| 5 | 0.872 | 0.893 | 0.759 | 0.959 | 0.857 | 0.714 |
| **평균** | **0.887** | **0.850** | **0.742** | **0.875** | **0.849** | **0.711** |

> 일부 fold에서 Test Sen=1.0이 나오는 것은 test fold당 ~14명(임상저하 ~7명)으로 매우 작기 때문.  
> 7명 전원 적중 시 Sen=1.0이 되며, 데이터 누출이 아닌 소표본 변동성. 100회 반복 평균에서는 Sen이 안정화된다.
...
| Subject (71명) — 100회 반복 CV 평균 | 0.861 ± 0.020 | 0.943 | 0.722 |
| **Threshold** | **0.345** | **Youden 기준** | **seed=42 OOF** |

- Threshold 전략: Youden index (sens + spec 최대화) → 0.345
...
| 그룹 | 보정 후 확률 범위 | threshold=0.345 |
|------|------------------|-----------------|
| 정상 (6명) | 0.009 ~ 0.352 | 전원 정상 판정 ✓ |
| 저하 (2명) | 0.612 ~ 0.814 | 전원 저하 판정 ✓ |

→ 8/8 정답, 클린 재학습 Youden 임계값(0.345) 사용
...
| **v3 (최종)** | **jerk+HR 3개** | **0.861 ± 0.020 (subject CV)** | **임상 OR 라벨** | acc-only, VIF≤2.17 |

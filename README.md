# 파이널 프로젝트 데모
# 🏃 final inzi: 시니어 맞춤형 홈케어 낙상 예측 및 예방 운동 솔루션

디지털 바이오마커와 AI 어시스턴트를 활용하여, 일상생활 속 낙상 위험을 예측하고 시니어 맞춤형 치매 예방 운동을 지원하는 홈케어 솔루션입니다.

## 📌 핵심 문제 해결 (Troubleshooting)
**1. 이종 기기 간 센서 데이터 도메인 오차 극복**
* **문제:** 72시간 동안 허리에 착용한 IMU 관성 측정 장치 데이터와 실제 서비스 환경(스마트폰) 간의 물리적 센서 편차 발생.
* **해결 방안:** 3초간의 정지 상태를 중력 기준으로 삼아 축(Axis)을 매핑. 두 센서 원시 데이터의 최고/최저 스케일을 맞추기 위해 알파값을 곱하는 캘리브레이션 수행.
* **결과:** 실측 인원의 90% 이상이 보정값 범위 내에 안착하며 예측 신뢰도 확보.

**2. 센서 노이즈(Outlier) 제어 및 모델링**
* **문제:** 신체 움직임에 따른 순간적인 센서 튐 현상(Noise Spike)으로 시계열 파형 모델 성능 저하.
* **해결 방안:** 센서 데이터 전처리 시 **IQR Median** 알고리즘을 도입하여 찰나의 노이즈를 필터링하고 안정적인 시계열 파형 추출 후 모델링 (1D CNN, LSTM 적용 가능성 탐구).

**3. 시니어 맞춤형 UI/UX 및 STT 파인튜닝**
* **특징:** MOCA 디지털 설문 구현 및 OpenCV 기반 그림 채점 로직 구축. 일반 STT 모델이 시니어의 발음을 인식하기 어려운 점을 고려하여 파인튜닝 진행 및 맞춤형 설정값 부여. RAG 기반 LLM을 통해 센서 기반 응원 및 동적 UI/UX 제어.

## 🛠 Tech Stack
* **Language & Analysis:** Python, Pandas, NumPy
* **AI/ML:** PyTorch, OpenCV, RAG 기반 LLM, STT/TTS
* **App/Web:** Kotlin, Streamlit
## 최종 보행 모델 재현 파일 색인
...
### 회전(turn) window 처리 스모크 (2026-07-24)

집 환경용 60초 자유보행 구성을 탐색하기 위해, 회전 구간 window를 제거하는 방식이 유리한지 1회 스모크로 확인했다. **회전 전면 제거는 미채택**.  
공통 subjects n=68 (clean window 부족한 3명 제외), 1-repeat paired 평가.

| 방식 | AUC | 민감도 | 특이도 | 정확도 | F1 |
|------|----:|------:|------:|------:|---:|
| 중앙 40초 16개 top3 | 0.844 | 0.846 | 0.621 | 0.750 | 0.795 |
| **60초 전체 26개 top3** | 0.837 | 0.821 | **0.759** | **0.794** | **0.821** |
| 60초 회전 window 제거 top3 | 0.827 | 0.821 | 0.655 | 0.750 | 0.790 |

**실험 결론**

- **회전 window 전면 제거 시 오히려 하락** — AUC 0.837 → 0.827, 특이도 0.759 → 0.655. 회전 구간이 순수 잡음이 아니라 일상 운동능력 정보를 일부 담고 있거나, 제거하면서 유효 보행 정보까지 같이 버려진 것으로 보임.
- **clean window 현황**: 71명 중 평균 21.7/26개, 중앙값 26개, 최소 8개 확보 68/71명. 3명은 회전 영향이 커 4~6개만 남음 → 집 환경에서 회전 제거는 측정 무효화(실패율) 리스크도 있음.
- **gyro 역할 재정의**: 회전 삭제용이 아니라 ① 보행 자동 인식 ② 심한 센서 오류 검출 ③ 거의 전부 회전·정지인 비정상 측정 걸러내기 ④ 회전 횟수·품질 기록에 사용.
- **가장 유망한 구성**: 60초 자유보행 → 10초 window 26개 → 회전 제거하지 않고 top3 median/IQR → LR. AUC는 중앙 40초보다 0.007 낮지만 특이도·정확도·F1이 가장 좋고 집 사용 실패율이 낮음.
- **주의**: 단일 1회 평가이므로 확정 근거로는 약함. 세 방식의 AUC 차이(0.007~0.017)는 n=68 소표본에서 CI가 겹칠 수 있어, **60초 전체 26개 top3를 100회 nested CV로 검증**한 뒤 확정해야 함.
- 관련: `analysis_scripts/smoke_turn_clean_pooling_60s.py`, 결과 `analysis_outputs/smoke_turn_clean_pooling_60s/`

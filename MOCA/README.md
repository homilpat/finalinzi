...
- **신체 평가**: 스마트폰 IMU 센서(가속도계+자이로스코프) 기반 보행 스크리닝 — 온디바이스 ML 추론
...
| 평가 항목 | 기술 |
|---|---|
| IMU 기반 보행 분석 | 가속도계 + 자이로스코프 (100 Hz) |
| 온디바이스 신호처리 | Butterworth 밴드패스 필터 (보행 주파수 추출) |
| 온디바이스 특징 추출 | 보행 진폭·주기성 수치화 (4개 feature) |
| 수집 프로토콜 | 30초 걷기 → 최적 10초 구간 자동 추출 |
| 온디바이스 ML 추론 | 로지스틱 회귀 — 서버 없이 실시간 분류 (AUC 0.845) |

...
def report_detail_page():
    member_id = _current_member_id()
    logs = session.get('demo_exercise_logs', [])
    if not logs and member_id:
        with get_conn() as conn:
            rows = conn.execute(
                """
    ...
    exercise_data = _personal_exercise_data()
    if session.get('demo_mode'):
        exercise_data['report']['exercise_minutes'] = 40

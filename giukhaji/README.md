...
- **AI 코치**: 하이브리드 AI 코치 펭트 — 규칙기반 답변 우선, 필요 시 LLM API(OpenAI gpt-4.1-mini). 발화 키워드 규칙 라우터로 TTS 속도·글자 크기 등 앱 설정을 LLM 없이 즉시 조절
...
| 서버 신호처리 | 축 정렬(V/ML/AP) + 100 Hz 리샘플 + 진폭 보정 α = 1.9705 + 0.6~3.0 Hz 4차 Butterworth 밴드패스 |
| 특징 추출 | `v_jerk_rms_median`(움직임 충격), `v_jerk_rms_iqr`(움직임 충격 변동성), `v_harmonic_ratio_iqr`(좌우 보행 대칭성 변동성) |
| 수집 프로토콜 | 20초 이상 걷기 → 20s 구간 안 10s 서브윈도우(2s 스텝) Median·IQR 집계 |
| ML 추론 | 로지스틱 회귀 — acc-only 3피처 모델. 5-fold × 100회 반복(fold별 민감도 ≥ 0.80 중 특이도 최대 임계값) AUC 0.873 ± 0.007 · 민감도 0.835 · 특이도 0.731, 서비스 임계값 0.50 고정 |

### 공통 기능

| 기능 | 기술 |
|---|---|
| 하이브리드 AI 코치 | 규칙기반 답변 우선 → 필요 시 LLM API (OpenAI gpt-4.1-mini), 키가 없거나 호출 실패 시 규칙 답변으로 복귀 |
| 실시간 UI 동적 제어 | 발화 키워드 규칙 라우터(`getPengteuCommand`, LLM 미경유) → 프로필 저장 + Android 네이티브 API (TTS 속도·볼륨·글자 크기·고대비·움직임 줄이기) |
| 맥락 인식 자동 대응 | 검사 화면 무입력 10초·STT 2회 연속 실패 감지 → 펭트가 먼저 안내 |
| 지식 검색·주입 | TF-IDF RAG (논문·운영 문서 청크, 출처·페이지·DOI 표기) |
